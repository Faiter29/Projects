...
void appendNode(List *list, const int val) {
    ...
    while (nieco){
        std::cout<<nieco->data;
        if (nieco->next!= nullptr)std::cout<<",";
        nieco=nieco->next;
    }
}
...
List *createList(const ListData *list_data) {
    List *list = new List;
    if (list_data->len <= 0) list->first = nullptr;
    else {
...
void insertNode(List *sorted_list, const int val) {
    Node *nieco = sorted_list->first;
    Node *to=create(val);
    //List *list=new List;
    if (val < nieco->data) {//zaciatok
        to->next=nieco;
        sorted_list->first=to;
    }
    else {//stred
        nieco = sorted_list->first;
        while (nieco->next != nullptr) {
            //to=nieco;
            if (nieco->data <= val && nieco->next->data>=val) {
                to->next=nieco->next;
                nieco->next=to;
                break;
            }
...
List *joinLists(List *list1, List *list2) {
    ...
    if(list1->first== nullptr){
        ...
        while(daco->next!= nullptr) {
            daco = daco->next;
        }
        if (list2->first!= nullptr){
            daco->next=list2->first;
        }
...
bool isPalindrome(const List *list) {
    ...
    int pocet=x/2;
    for (int i = 0; i < x/2; i++) {
        if(a[i]==a[x-i-1]) pocet--;
        std::cout <<pocet;
    }
    bool suhrn;
    if (pocet==0) suhrn= true;
    else suhrn= false;

    return suhrn; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
int sumNodes(const List *list, const int n) {
    ...
    return vysledok; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
Node *findLastNodeOccurrence(const List *list, const int val) {
    ...
    int *adresa= nullptr;
    while (daco!= nullptr){
        if(daco->data==val) adresa=addressof(daco->data);
        daco=daco->next;
    }
    return (Node*) adresa; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
