...
{
    size_t LeftChild = topIndex * 2 + 1;
    size_t RightChild = topIndex * 2 + 2;
    size_t Swap = topIndex;
    ...
    if (RightChild < length && data[RightChild] < data[Swap]) {
        Swap = RightChild;
    }
    if (Swap != topIndex) {
        swap(&data[topIndex], &data[Swap]);
        siftDown(data, Swap, length);
    }
}
...
int main() {
    ...
    //int pole2[]={100, 8, 2, 1, 0, 5, 6, 7, 4, 2, 3, 11, 12, 13, 14, 15, 16, 17};
    //buildHeapSiftDown(pole,11);
    //heapSort(pole,11);
    //siftDown(pole2,1,18);
    //for (int i=0;i<18;i++){
    //    std::cout<<pole2[i]<<", ";
