...
queue<int> reverse(const list<int> & data) noexcept {
    ...
    return hlavny; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
vector<vector<int>> create2dVector(const size_t size1, const size_t size2, const int value) noexcept {
    ...
    for(size_t i=0;i<size1;i++){
        ...
        for (size_t j = 0; j < size2; j++) {
            pomoc.push_back(value);
        }
        meno.push_back(pomoc);
    }
    return meno; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
string connectWithSpace(const string & text, const int number) noexcept {
    ...
    konec=text;
    konec+=" ";
    konec+=to_string(number);
    return konec; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
int sum(istringstream & text) noexcept {
    ...
    return cislo; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
list<string> translate(const map<string, string> & translator, const list<string> & sentence) noexcept {
    list<string> preklad;
    for (auto daco = sentence.begin(); daco != sentence.end(); daco++) {
        if ( translator.find(*daco) == translator.end() ) preklad.push_front("?");
        else preklad.push_front(translator.find(*daco)->second);
    }
    preklad.reverse();
    //for (auto it = preklad.begin(); it != preklad.end(); it++) {
    //    cout << *it;
   // }
    return preklad; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
bool areParenthesisPaired(const string & mathExpression) noexcept {
    ...
    for(size_t i=0;i<mathExpression.length();i++) {
        if (mathExpression[i] == '(') a++;
        if (mathExpression[i] == '[') b++;
        if (mathExpression[i] == '{') c++;
        if (mathExpression[i] == '<') d++;
        if (mathExpression[i] == ')') a--;
        if (mathExpression[i] == ']') b--;
        if (mathExpression[i] == '}') c--;
        if (mathExpression[i] == '>') d--;
    ...
    if(a==0 && b==0 && c==0 && d==0){
        for(size_t i=0;i<mathExpression.length();i++) {
            if (mathExpression[i] == '(' || mathExpression[i] == '{' || mathExpression[i] == '[' || mathExpression[i] == '<') {
                for (size_t j = i; j < mathExpression.length(); j++) {
                    if (mathExpression[j] == '(' || mathExpression[j] == '{' || mathExpression[j] == '[' || mathExpression[j] == '<') pocet++;
                    else if (mathExpression[j] == ')' || mathExpression[j] == '}' || mathExpression[j] == ']' || mathExpression[j] == '>') pocet--;
                    if (pocet==0){
                        if (mathExpression[i] == '(' && mathExpression[j] == ')') hodnota=true;
                        else if (mathExpression[i] == '{' && mathExpression[j] == '}') hodnota=true;
                        else if (mathExpression[i] == '[' && mathExpression[j] == ']') hodnota=true;
                        else if (mathExpression[i] == '<' && mathExpression[j] == '>') hodnota=true;
                        else hodnota=false;
                        break;
            ...
            if (pocet!=0) hodnota=false;
            if (hodnota== false) break;
        }
    }
    else hodnota=false;
    return hodnota; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
int execute(int (*function)(int, int), int parameter1, int parameter2) {
    // TODO
    int cislo=(*function)(parameter1,parameter2);
    return cislo; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
void throwInvalidArgument() {
    throw std::invalid_argument("INVALID_ARGUMENT"); // konstruktor vynimky ma parameter
}
...
void throwOutOfRange() {
    throw std::out_of_range("UNKNOWN_EXCEPTION");
}
...
list<int> createList(const list<int>::size_type length, const list<int>::size_type indexOfOne) {
    ...
    if(indexOfOne>=length){
        ...
        for (size_t i = 0; i < length; i++) {
            if(i==indexOfOne) daco.emplace_back(1);
            else daco.emplace_back(0);
        }
    }
    return daco; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
