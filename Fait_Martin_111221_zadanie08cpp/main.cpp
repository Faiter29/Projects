...
list<int>::const_iterator findValue(const list<int> & data, int value) noexcept {
    list<int>::const_iterator daco= find (data.cbegin(), data.cend(), value);
...
bool isInRange200to400(const int element) noexcept {
    bool hodnota;
    if(element>=200 && element<=400) hodnota=true;
    else hodnota=false;
    return hodnota;
}

list<int>::const_iterator findInRange200to400(const list<int> & data) noexcept {
    list<int>::const_iterator daco= find_if(data.cbegin(),data.cend(), isInRange200to400);
    return daco; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
list<int> add(const list<int> & data1, const list<int> & data2) noexcept {
    list<int> daco(data1.size());
    transform(data1.begin(),data1.end(),data2.cbegin(),daco.begin(),plus<int>());
    return daco; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
void sort2(vector<int> & data) noexcept {
    stable_sort(data.rbegin(),data.rend(), less<int>());
    }
