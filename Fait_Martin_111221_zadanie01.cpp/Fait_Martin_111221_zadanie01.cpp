...
POKYNY:
(1)  Subor premenujte na Priezvisko_Meno_ID_zadanie01.cpp (pouzite vase udaje bez diakritiky). ok
...
int numDigits(int value) {
    string cislo=to_string(value);
    int hodnota=0;
    while(cislo[hodnota]!='\0'){
        hodnota++;
    }
    return hodnota;
}
...
/*
    ...
    PARAMETRE:
        - [in] date - datum
        - [in] format - format vytlacenia datumu
...
void print(const Date *date, const char *format) {
    int i=0;
    while(format[i]!='\0'){
        if (format[i]=='D') std::cout<<date->day;
        else if (format[i]=='M') std::cout<<date->month;
        else if (format[i]=='Y') std::cout<<date->year;
...
bool isInLeapYear(const Date *date) {
    // TODO
    bool uvaha= false;
    if((date->year%4==0 && date->year%100!=0) || date->year%400==0) uvaha= true;
    return uvaha; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
...
bool isValid(const Date *date) {
    bool uvaha= false;
    if(date->month<=12 && date->month>0 && date->day>0 && date->day<=31){
    if(date->month%2==1 && date->day<=31 && date->month<8) uvaha= true;
    if(date->month%2==0 && date->month!=2 && date->day<=30 && date->month<8) uvaha= true;
    if(date->month%2==0 && date->day<=31 && date->month>=8) uvaha= true;
    if(date->month%2==1 && date->day<=30 && date->month>=8) uvaha= true;
    if(date->month==2 && date->day<=29 && ((date->year%4==0 && date->year%100!=0) || date->year%400==0)) uvaha= true;
    if(date->month==2 && date->day<=28 && ((date->year%4!=0 && date->year%100==0) || date->year%400!=0)) uvaha= true;
    }
    return uvaha; // tento riadok zmente podla zadania, je tu len kvoli kompilacii
}
