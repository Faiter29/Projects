...
int min(const BinarySearchTree *tree) {
    if (!tree) throw ValueNotExistsException();
    Node *akt=tree->root;
    while(akt->smaller!= nullptr) akt=akt->smaller;
    return akt->value;
}
...
size_t preOrder(Node *smt,size_t result){
    if(smt){
        result++;
        result=preOrder(smt->smaller,result);
        result=preOrder(smt->greater,result);
    }
    return result;
}

size_t count(const BinarySearchTree *tree) noexcept {
    size_t result = preOrder(tree->root,0);
    return result;
}
